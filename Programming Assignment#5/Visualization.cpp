...
void SetCamera()
{
    ...
    if (camera.ViewMode == camera.Perspective)
    {
        XYZ eyeXYZ
        {
            static_cast<float>(-2 * eye.r * Sin(eye.theta) * Cos(eye.phi)),    // x
            static_cast<float>(2 * eye.r * Sin(eye.phi)),                    // y
            static_cast<float>(2 * eye.r * Cos(eye.theta) * Cos(eye.phi))    // z
        };
...
void WindowKey(unsigned char key, int x, int y)
{
    ...
    {
        VisOpt.Camera.ViewPoint.r += 1.0;
        VisOpt.Camera.FovAngle += 10.0;
    }
    else if (key == '+' && key < 179)
    {
        VisOpt.Camera.ViewPoint.r -= 1.0;
    ...
    {
        VisOpt.Camera.Translation.y += 1.0;
    }
    else if (key == '2')
    {
        VisOpt.Camera.Translation.y -= 1.0;
    ...
    {
        VisOpt.Camera.Translation.x -= 1.0;
    }
    else if (key == '6')
    {
        VisOpt.Camera.Translation.x += 1.0;
...
{
    GLfloat mat_ambient[4] = { 0.0, 0.5, 1.0, 1.0 };
    GLfloat mat_diffuse[4] = { 0.0, 0.5, 1.0, 1.0 };
    GLfloat mat_specular[4] = { 0.5, 0.5, 1.0, 1.0 };
    ...
    GLfloat mat_ambient2[4] = { 1.0, 0.5, 0.0, 1.0 };
    GLfloat mat_diffuse2[4] = { 1.0, 0.5, 0.0, 1.0 };
    GLfloat mat_specular2[4] = { 1.0, 0.5, 0.0, 1.0 };
